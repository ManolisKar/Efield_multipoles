...
for i in range(len(r)):
    if math.sqrt(math.pow(r[i]-711.2,2)+math.pow(z[i],2))<4.55 and abs(th[i]-6.45)<0.05 and abs(V[i])>0.01: 
    #and math.sqrt(math.pow(r[i]-711.2,2)+math.pow(z[i],2))>2\ # perhaps avoid inner region, since the V formula is based on boundary conditions at edges
        for item in data[i]:
            f.write('%.11E  '%item, )
        f.write('\n')
        accepted_entries += 1
